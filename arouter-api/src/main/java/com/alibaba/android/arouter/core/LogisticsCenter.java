        ...
        try {
            ...
            if (registerByPlugin) {
                ...
                if (ARouter.debuggable() || PackageUtils.isNewVersion(context)) {
                    ...
                    // 这一段代码是遍历apk中所有的dex文件，找到包名com.alibaba.android.arouter.routes下的所有类
                    // ，这些类是通过 arouter-compiler 注解处理器编译时生成的
                    routerMap = ClassUtils.getFileNameByPackageName(mContext, ROUTE_ROOT_PAKCAGE);
                    if (!routerMap.isEmpty()) {
                        // 把获取到的组件信息存到sharedPreferences中。
                ...
                // 通过获取到的routerMap集合，去实例化所有的路由类。其中包括，页面，服务和拦截器分别对应IRouteRoot，IProviderGroup和IInterceptorGroup
        ...
        if (null == routeMeta) {
            ...
            switch (routeMeta.getType()) {
                case PROVIDER:  // if the route is provider, should find its instance
                    // 根据routeMeta中的Destination对象来创建IProvider,（服务类，拦截器实例(拦截器也继承IProvider，也是特殊服务类)都在这被创建）
