```
  基于阿里的arouter,主要做了以下修改
       1.增加源码中文注释，
       2.升级androidX ，
       3.更新到最新版gradle -plugin
       4.移除相关无用的依赖,修改gradle脚本
    
  简介：一个用于帮助 Android App 进行组件化改造的框架 —— 支持模块间的路由、通信、解耦
...
&nbsp;
arouter-gradle-plugin 是 Gradle插件脚本的源码。作用是通过编译时自动注册组件  
&nbsp;
module-java和module-kotlin是两个demo库分别用java和Kotlin来实现。主要是一些使用上的区别  
&nbsp;


模块|arouter-api|arouter-compiler|arouter-gradle-plugin
---|---|---|---
...
13. **支持第三方 App 加固**(使用 arouter-gradle-plugin 实现自动注册)
14. **支持生成路由文档**
...
6. 使用 Gradle 插件实现路由表的自动加载 (可选)
    ...
    buildscript {
        ...
        dependencies {
            classpath "com.alibaba:arouter-gradle-plugin:?"
        }
    }
    ```
